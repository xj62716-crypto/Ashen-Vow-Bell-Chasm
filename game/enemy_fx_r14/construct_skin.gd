...
func _ready() -> void:
	...
	material.albedo_texture=load("res://enemy_fx_r14/stone_color.jpg");material.uv1_triplanar=true;material.uv1_world_triplanar=true;material.uv1_scale=Vector3.ONE*.65
	material.normal_enabled=true;material.normal_texture=load("res://enemy_fx_r14/stone_normal.jpg");material.normal_scale=.23
	...
	if kind==&"anchor":
		for index in range(3):
			var ring:=TorusMesh.new();ring.inner_radius=.27;ring.outer_radius=.315;ring.rings=32;ring.ring_segments=8
			var mesh:=MeshInstance3D.new();mesh.mesh=ring;mesh.material_override=rune_material;mesh.rotation=Vector3(index*.8,index*.65,.3);add_child(mesh);sigils.append(mesh)
		var core:=SphereMesh.new();core.radius=.12;core.height=.24;var m:=MeshInstance3D.new();m.mesh=core;m.material_override=material;add_child(m)
	else:
		...
		if kind==&"wall":
			for side in [-1,1]:
				for column in range(5): _rune(Vector3(side*(dimensions.x*.5+.004),0,(float(column)/4.-.5)*dimensions.z*.84),Vector3.RIGHT,.22)
		else:
